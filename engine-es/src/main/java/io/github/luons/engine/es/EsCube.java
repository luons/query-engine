...
public class EsCube extends AbstractSqlCube {
        ...
        cubeMapList.sort(new Comparator<CubeMap<Object>>() {
            ...
            public int compare(CubeMap<Object> o1, CubeMap<Object> o2) {
                for (String orderField : orderSet) {
                    ...
                    if (!o1.containsKey(field) || !o2.containsKey(field) || o1.getOrDefault(field, (null)) == null
                            || o2.getOrDefault(field, (null)) == null) {
                        continue;
                    }
    ...
    private List<Map<String, Object>> queryDbByAggs(Query query) {
        ...
        if (dimensions != null && dimensions.size() > 0) {
            ...
            for (int i = (dimensionList.size() - 1); i >= 0; i--) {
                ...
                if (aggMap.size() > 0) {
                    ((Map<String, Object>) tmpMap.get(dimension)).put("aggs", aggMap);
                }
                aggMap = tmpMap;
            }
        ...
        return esClient.queryDslForAggs((index + "-*"), "", JacksonUtils.toJsonString(dslObject));
    }

    private List<Map<String, Object>> queryDbByDsl(Query query) {
        Map<String, String> scrollMap = new HashMap<>();
        return esClient.queryDsl((index + "-*"), "", JacksonUtils.toJsonString(queryToDsl(query)), scrollMap);
    }
    ...
    // TODO $符号需要加双引号
    private List<Map<String, Object>> queryDbBySql(Query query) {
    ...
    public Map<String, Object> getAggMap(String field) {
        Map<String, Object> aggMap = new HashMap<>();
    ...
    public static void main(String[] args) {
        ...
        set.add("$event");
        query.setFields(set);
        System.out.println(JacksonUtils.toJsonString(queryToDsl(query)));
    }
}
